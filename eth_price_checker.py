...
def discord_setup_hook(eth_rate, currency):
     # discord webhook setup
    webhook = DiscordWebhook(url=data["discord_hook_url"])
    embed = DiscordEmbed(title='Script is running sucessfully', color='17bf44')
    embed.set_thumbnail(url = data["image"])
    embed.set_footer(text= "ETH Price Monitoring by Picooo")
    embed.set_timestamp()
    embed.add_embed_field(name='Current Ethereum Price', value=f'{eth_rate} {currency}')
    ...
    webhook.add_embed(embed)
    response = webhook.execute()
def discord_setup_hook_fail():
     # discord webhook setup
    webhook = DiscordWebhook(url=data["discord_hook_url"])
    embed = DiscordEmbed(title='Script is not running. Fail please check', color='a83232')
    embed.set_footer(text= "ETH Price Monitoring by Picooo")
    embed.set_timestamp()
    webhook.add_embed(embed)
...
while True:
    ...
    if is_script_running == True:
        discord_setup_hook(eth_rate_eur, currency)
    else: 
        discord_setup_hook_fail()
        break
